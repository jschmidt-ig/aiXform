...
## How to visualize the model (recommended options)

### Option A (best for a single infographic): 2D “Depth vs Breadth” map
- **X-axis (Breadth):** Snippet → File → Story → Feature → Milestone
- **Y-axis (Depth):** Autocomplete → Supervised implementer → PR owner → Specialist team → Hierarchical org
- Place the 5 levels along the diagonal.
- Add “badges” around Levels 3–5 (model routing, council, RAG, voice, evals).

### Option B: Maturity “stairs” + sidebars
- Stairs show the 5 levels.
- Two sidebars:
  - “Human role changes”
  - “Quality signal changes”
- A small “productivity curve” inset chart.

### Option C: Layered stack (governance on top)
- Bottom: Autocomplete
- Middle: Agents + PRs
- Top: Orchestration + governance/evals
